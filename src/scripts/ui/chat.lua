...
starmourn.ui.chat.currenttab = starmourn.ui.chat.currenttab or starmourn.ui.chat.config.alltab or starmourn.ui.chat.config.channels[1]
...
function starmourn.ui.chat:create()
  ...
  for i, tab in ipairs(starmourn.ui.chat.config.channels) do
    starmourn.ui.chat.tabs[tab] = Geyser.Label:new({ name = string.format('chattab%s', tab) }, starmourn.ui.chat.tabbox)
...
function starmourn.ui.chat:append(chat)
  ...
  if starmourn.ui.chat.config.blink and chat ~= starmourn.ui.chat.currenttab then
    if (starmourn.ui.chat.config.alltab == starmourn.ui.chat.currenttab) and not starmourn.ui.chat.config.blinkonall then
      return
    else
...
function starmourn.ui.chat:blink()
  if starmourn.ui.chat.blinkID then killTimer(starmourn.ui.chat.blinkID) end
  ...
  starmourn.ui.chat.blinkID = tempTimer(starmourn.ui.chat.config.blinktime, function() starmourn.ui.chat:blink() end)
end
...
function starmourn.ui.chat.handle_comm_channel_text()
  ...
  if starmourn.ui.chat.config.filterccc and (not gmcp.Comm.Channel.Players or (not table.contains(gmcp.Comm.Channel.Players, talker) and not (talker == gmcp.Char.Name.name))) then return end
