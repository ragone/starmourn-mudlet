...
function starmourn.ui.info:create()
	...
	for i, tab in ipairs(starmourn.ui.info.config.types) do
		starmourn.ui.info.tabs[tab] = Geyser.Label:new(
			{ name = string.format("infotab%s", tab) },
			starmourn.ui.info.tabbox
		)
...
function starmourn.ui.info:dechoLink(tab, message, command, hint)
	...
	if starmourn.ui.info.config.preservebackground then
		dechoLink(
			string.format("infowin%s", tab),
			string.format("<:%s,%s,%s>%s", r, g, b, message),
			command,
			hint,
			true
		)
...
function starmourn.ui.update_tasks()
	...
	for _, task in pairs(tasks.completed) do
		...
		c = c + 1
		if c == starmourn.ui.info.config.tasklimit then
			break
		end
	end
end
...
function starmourn.ui.update_room()
	...
	for _, entity in pairs(starmourn.ui.info.room_mobs) do
		...
		local colors = isTarget and { starmourn.ui.info.config.dash_color, starmourn.ui.info.config.dash_color }
			or {
				starmourn.ui.info.config.dash_color,
				noTarget and starmourn.ui.info.config.text_color or "<0,175,215>",
			}
		...
		starmourn.ui.info:decho(
			...
			string.format(
				...
				colors[1],
				isTarget and ">" or "",
				starmourn.ui.info.config.displayIDs
						and string.format("%s%d ", color(isTarget and "IndianRed" or "MediumSlateBlue"), entity.id)
					or ""
			)
		)
			...
			for _, containerItem in pairs(starmourn.ui.info.containers[data.id]) do
				starmourn.ui.info:decho(
					...
					string.format(
						...
						color("DimGrey"),
						starmourn.ui.info.config.displayIDs
								and string.format("%s%d ", color("MediumSlateBlue"), containerItem.id)
							or ""
					)
				)
				local labels = {
...
function starmourn.ui.info.update_inventory()
	...
	for id, entity in
		spairs(inventory.wearing, function(t, a, b)
			return string.lower(t[a].name) < string.lower(t[b].name)
		end)
	do
		starmourn.ui.info:decho(
			"Inventory",
	...
	for name, entity in spairs(inventory.items) do
		starmourn.ui.info:decho(
			...
			string.format(
				"\n %s- %s",
				starmourn.ui.info.config.dash_color,
				starmourn.ui.info.config.displayIDs and string.format("%s%d ", color("MediumSlateBlue"), entity.id)
					or ""
			)
		)
		...
		dechoPopup(
			...
			string.format(
				...
				starmourn.ui.info.config.text_color,
				entity.name,
				entity.count > 1 and string.format(" (x%d)", entity.count) or ""
			),
			labels,
			cmds,
			true
	...
	if
		next(inventory.wielding) ~= nil
		or next(inventory.wearing) ~= nil
		or next(inventory.containers) ~= nil
		or next(starmourn.ui.info.armor) ~= nil
	then
		starmourn.ui.info:cecho("Inventory", "\n\n")
	end
	...
	for id, entity in pairs(inventory.containers) do
		...
		dechoPopup(
			...
			string.format(
				...
				starmourn.ui.info.config.text_color,
				entity.name,
				starmourn.ui.info.containers[id] and next(starmourn.ui.info.containers[id]) == nil and " (Empty)" or ""
			),
			labels,
			cmds,
			true
...
starmourn.utils.events.register(
	"room removeplayer",
	"gmcp.Room.RemovePlayer",
	"starmourn.ui.info.handle_room_remove_player"
)
